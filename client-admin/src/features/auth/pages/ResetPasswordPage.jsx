...
export const ResetPasswordPage = () => {
    ...
    const handleReset = () => {
        if (!password || password !== confirmar) return;
        setLoading(true);
    ...
    return (
        ...
        <div className="relative w-full max-w-sm mx-auto min-h-screen md:min-h-0 flex flex-col">
            ...
            <div className="relative z-10 flex flex-col items-center px-7 pt-10 pb-10 gap-1 flex-1">
                ...
                {done ? "¡Contraseña actualizada! Ya puedes iniciar sesión." : "Ingresa tu nueva contraseña."}
                </p>

                <div className="rounded-full border-[3px] bg-white overflow-hidden flex items-center justify-center shadow-md mb-6" style={{ borderColor: "#f0c030", width: 110, height: 110 }}>
                ...
                <>
                    ...
                    <div className="flex items-center bg-white rounded-2xl border border-gray-200 shadow-sm px-4 py-3 gap-3">
                        <LockIcon />
                        <input type="password" placeholder="Nueva contraseña" value={password} onChange={(e) => setPassword(e.target.value)} className="flex-1 outline-none text-gray-700 text-sm bg-transparent placeholder-gray-400" />
                    </div>
                    <div className="flex items-center bg-white rounded-2xl border border-gray-200 shadow-sm px-4 py-3 gap-3">
                        <LockIcon />
                        <input type="password" placeholder="Confirmar contraseña" value={confirmar} onChange={(e) => setConfirmar(e.target.value)} className="flex-1 outline-none text-gray-700 text-sm bg-transparent placeholder-gray-400" />
                    </div>
                    </div>
                    <button
                    onClick={handleReset}
                    ...
                    >
                    {loading ? <Spinner size={20} color="#fff" /> : "Restablecer contraseña"}
                    </button>
                </>
                )}

                {done && (
                <a href="/" className="w-full py-3.5 rounded-2xl font-semibold text-white text-base shadow-md mt-4 flex items-center justify-center active:scale-95 transition-transform" style={{ background: "linear-gradient(135deg,#f5c518 0%,#e6a800 100%)" }}>
                    Ir al Login
                </a>
                )}
            </div>
